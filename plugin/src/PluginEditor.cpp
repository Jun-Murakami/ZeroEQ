...
#ifndef ZEROEQ_VERSION_STRING
 #define ZEROEQ_VERSION_STRING "0.0.0-dev"
#endif
                  ...
                  .withNativeFunction(
                             ...
                             juce::WebBrowserComponent::NativeFunctionCompletion completion)
                      {
                          ...
                          if (args.size() > 0)
                          {
                              ...
                              // WebUI 読込時。args = (innerWidth, innerHeight, devicePixelRatio)。
                              if (action == "apply_layout" && args.size() >= 3)
                              {
                                  const double cssW = static_cast<double>(args[1]);
                                  const double cssH = static_cast<double>(args[2]);
                                  // Win/mac は従来どおり ratio = 論理px/CSS px（通常 1）を維持してリサイズ換算に使う。
                                  webResizeRatioW = (cssW > 0.0) ? static_cast<double>(getWidth())  / cssW : 1.0;
                                  webResizeRatioH = (cssH > 0.0) ? static_cast<double>(getHeight()) / cssH : 1.0;
                              ...
                              if (action == "resizeTo" && args.size() >= 3)
                              {
                                  ...
                                  // args は CSS px。CSS(設計)空間でクランプ → ratio を掛けて論理 px へ（Linux は ratio==1）。
...
void ZeroEQAudioProcessorEditor::resized()
{
    ...
    // 編集サイズを APVTS state（in-memory ValueTree）へ控える。実際のディスク永続化は host の
    //  getStateInformation（= プロジェクト保存／クローズ時）でのみ起きるので、ここでの setProperty は
    //  「保存」ではなく現在値の追従にすぎない（保存しっぱなしにはならない）。
    //  Linux は peerScale を DPR に固定済みのため getWidth() == CSS px。Win/mac は従来どおり論理px。
...
void ZeroEQAudioProcessorEditor::applyDisplayScale()
{
#if JUCE_LINUX || JUCE_BSD
...
void ZeroEQAudioProcessorEditor::setScaleFactor(float /*newHostScale*/)
{
    applyDisplayScale();
}
