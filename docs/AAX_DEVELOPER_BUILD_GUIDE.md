...
1. **依存ライブラリの確認**
   ...
   depends.exe ZeroEQ.aaxplugin

   # macOS
   otool -L ZeroEQ.aaxplugin/Contents/MacOS/ZeroEQ
   ```
...
2. **ログ機能の実装**
   ...
   // AAXプラグイン内でのログ
   #ifdef DEBUG
   AAX_TRACE(AAX_TRACE_PRIORITY_NORMAL, "ZeroEQ: %s", message);
   #endif
   ```
