...
# ZeroEQ macOS Release Build Script (zsh)
...
echo_header "ZeroEQ ${VERSION} Build Script (macOS zsh)"
...
EXISTING_ARTIFACTS_DIR="${BUILD_DIR}/plugin/ZeroEQ_artefacts/${CONFIGURATION}"
...
if [[ ${BUILD_AAX} -eq 1 ]]; then
    cmake --build "${BUILD_DIR}" --config "${CONFIGURATION}" --target ZeroEQ_VST3 ZeroEQ_AU ZeroEQ_Standalone ZeroEQ_CLAP ZeroEQ_AAX
else
    cmake --build "${BUILD_DIR}" --config "${CONFIGURATION}" --target ZeroEQ_VST3 ZeroEQ_AU ZeroEQ_Standalone ZeroEQ_CLAP
fi
echo_success "Plugin build completed"

# Artifact paths
ARTIFACTS_DIR="${BUILD_DIR}/plugin/ZeroEQ_artefacts/${CONFIGURATION}"

# Verify Universal Binary
echo_step "Checking architecture of built plugins..."
if [[ -f "${ARTIFACTS_DIR}/VST3/ZeroEQ.vst3/Contents/MacOS/ZeroEQ" ]]; then
    echo "  VST3:"
    lipo -info "${ARTIFACTS_DIR}/VST3/ZeroEQ.vst3/Contents/MacOS/ZeroEQ"
fi
if [[ -f "${ARTIFACTS_DIR}/AU/ZeroEQ.component/Contents/MacOS/ZeroEQ" ]]; then
    echo "  AU:"
    lipo -info "${ARTIFACTS_DIR}/AU/ZeroEQ.component/Contents/MacOS/ZeroEQ"
fi
if [[ -f "${ARTIFACTS_DIR}/Standalone/ZeroEQ.app/Contents/MacOS/ZeroEQ" ]]; then
    echo "  Standalone:"
    lipo -info "${ARTIFACTS_DIR}/Standalone/ZeroEQ.app/Contents/MacOS/ZeroEQ"
fi
if [[ -f "${ARTIFACTS_DIR}/CLAP/ZeroEQ.clap/Contents/MacOS/ZeroEQ" ]]; then
    echo "  CLAP:"
    lipo -info "${ARTIFACTS_DIR}/CLAP/ZeroEQ.clap/Contents/MacOS/ZeroEQ"
fi
if [[ ${BUILD_AAX} -eq 1 ]] && [[ -f "${ARTIFACTS_DIR}/AAX/ZeroEQ.aaxplugin/Contents/MacOS/ZeroEQ" ]]; then
    echo "  AAX:"
    lipo -info "${ARTIFACTS_DIR}/AAX/ZeroEQ.aaxplugin/Contents/MacOS/ZeroEQ"
fi
echo_success "Architecture verification completed"
SRC_VST3="${ARTIFACTS_DIR}/VST3/ZeroEQ.vst3"
SRC_AU="${ARTIFACTS_DIR}/AU/ZeroEQ.component"
SRC_APP="${ARTIFACTS_DIR}/Standalone/ZeroEQ.app"
SRC_CLAP="${ARTIFACTS_DIR}/CLAP/ZeroEQ.clap"
...
# AAX check
if [[ ${BUILD_AAX} -eq 1 ]]; then
    SRC_AAX="${ARTIFACTS_DIR}/AAX/ZeroEQ.aaxplugin"
    if [[ ! -d "${SRC_AAX}" ]]; then echo_error "AAX not found: ${SRC_AAX}"; exit 1; fi
fi

echo_step "Collecting artifacts..."
DEST_VST3="${OUTPUT_DIR}/ZeroEQ.vst3"
DEST_AU="${OUTPUT_DIR}/ZeroEQ.component"
DEST_APP="${OUTPUT_DIR}/ZeroEQ.app"
DEST_CLAP="${OUTPUT_DIR}/ZeroEQ.clap"
...
if [[ ${BUILD_AAX} -eq 1 ]]; then
    DEST_AAX="${OUTPUT_DIR}/ZeroEQ.aaxplugin"
    rm -rf "${DEST_AAX}"
...
if [[ -f "${DEST_VST3}/Contents/MacOS/ZeroEQ" ]]; then
    INFO="$(lipo -info "${DEST_VST3}/Contents/MacOS/ZeroEQ" 2>/dev/null || true)"
...
if [[ ${BUILD_AAX} -eq 1 ]]; then
   ...
   Copy the entire ZeroEQ.aaxplugin folder to the following location:
...
ZeroEQ ${VERSION} - macOS Installation Guide
====================================================
...
2. For VST3 Plugin:
   Copy the entire ZeroEQ.vst3 folder to the following location:
   ~/Library/Audio/Plug-Ins/VST3/

3. For Audio Unit (AU):
   Copy the entire ZeroEQ.component folder to the following location:
   ~/Library/Audio/Plug-Ins/Components/

4. For Standalone Application:
   Copy ZeroEQ.app to any preferred location, for example:
   /Applications/ or your Desktop.

5. For CLAP Plugin:
   Copy the entire ZeroEQ.clap folder to the following location:
...
PKG_ID_BASE="${PKG_ID_BASE:-com.bucketrelay.zeroeq}"
...
PKG_VST3="${PKG_WORK_DIR}/ZeroEQ_VST3.pkg"
pkgbuild \
    --root "${PKGROOT_VST3}" \
...
LOGIC_CATEGORY="${LOGIC_CATEGORY:-EQ}"
...
PKG_AU="${PKG_WORK_DIR}/ZeroEQ_AU.pkg"
pkgbuild \
    --root "${PKGROOT_AU}" \
...
PKG_APP="${PKG_WORK_DIR}/ZeroEQ_Standalone.pkg"
pkgbuild \
    --component "${DEST_APP}" \
...
PKG_CLAP="${PKG_WORK_DIR}/ZeroEQ_CLAP.pkg"
pkgbuild \
    --root "${PKGROOT_CLAP}" \
...
if [[ ${BUILD_AAX} -eq 1 ]]; then
    ...
    PKG_AAX="${PKG_WORK_DIR}/ZeroEQ_AAX.pkg"
    pkgbuild \
        --root "${PKGROOT_AAX}" \
...
fi
{
    ...
    echo "  <title>ZeroEQ ${VERSION}</title>"
    ...
    echo "  <pkg-ref id=\"${PKG_ID_BASE}.vst3\">ZeroEQ_VST3.pkg</pkg-ref>"
    echo "  <pkg-ref id=\"${PKG_ID_BASE}.au\">ZeroEQ_AU.pkg</pkg-ref>"
    echo "  <pkg-ref id=\"${PKG_ID_BASE}.app\">ZeroEQ_Standalone.pkg</pkg-ref>"
    echo "  <pkg-ref id=\"${PKG_ID_BASE}.clap\">ZeroEQ_CLAP.pkg</pkg-ref>"
    if [[ ${BUILD_AAX} -eq 1 ]]; then
        echo "  <pkg-ref id=\"${PKG_ID_BASE}.aax\">ZeroEQ_AAX.pkg</pkg-ref>"
    fi
...
PRODUCT_PKG_PATH="${OUTPUT_DIR}/../ZeroEQ_${VERSION}_macOS.pkg"
...
if [[ ${BUILD_AAX} -eq 1 ]]; then
    ZIP_NAME="ZeroEQ_${VERSION}_macOS_VST3_AU_AAX_CLAP_Standalone.zip"
else
    ZIP_NAME="ZeroEQ_${VERSION}_macOS_VST3_AU_CLAP_Standalone.zip"
fi
