...
if [[ ${BUILD_AAX} -eq 1 ]]; then
    cmake --build "${BUILD_DIR}" --config "${CONFIGURATION}" --target ZeroEQ_VST3 ZeroEQ_AU ZeroEQ_Standalone ZeroEQ_AAX
else
    cmake --build "${BUILD_DIR}" --config "${CONFIGURATION}" --target ZeroEQ_VST3 ZeroEQ_AU ZeroEQ_Standalone
fi
...
if [[ ${BUILD_AAX} -eq 1 ]]; then
    ZIP_NAME="ZeroEQ_${VERSION}_macOS_VST3_AU_AAX_Standalone.zip"
else
    ZIP_NAME="ZeroEQ_${VERSION}_macOS_VST3_AU_Standalone.zip"
fi
