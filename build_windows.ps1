    ...
    if (Test-Path $SourceAAX) {
        ...
        if (Test-Path $WrapToolPath) {
            ...
            $PfxCandidates = @(
                ...
                "$RootDir\zeroeq-dev.pfx",         # Project root
                "$env:USERPROFILE\.zeroeq\dev.pfx", # User home directory
                ".\certificates\zeroeq-dev.pfx"    # Certificates folder
            )
...
if ($BuildAAX) {
    $ZipName = "ZeroEQ_${Version}_Windows_VST3_AAX_Standalone.zip"
} else {
    $ZipName = "ZeroEQ_${Version}_Windows_VST3_Standalone.zip"
}
